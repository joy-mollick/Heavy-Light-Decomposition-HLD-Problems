...
void hld(int node , int par ) {
    ...
    BaseArray[indexNo] = nodevalue[node];///Base array holds the initially all node's value zero
...
int hldQuery(int u, int v) {
    ...
    while (true) {
        ...
        /// now node v and node u are in the same chain
            ...
            if (L <=R) ret += (get(L, R,1,vertex,1));///range(L,R) , start,end,root .
            return ret;
        }
...
ll query(int x, int y) {
    int l = lca(x, y);
    return (hldQuery(l, x)+hldQuery(l, y)-nodevalue[l]);
}

/// node i ,value of node i will be added by val
void update(int i, int val) {
    modify(indexInBaseArray[i], val,1,vertex,1);/// id,val,start,end,root
}
...
int main()
{
    ...
    for (int i = 1; i <= 20; i++)
        ...
        for(int i=1;i<=vertex;i++) {scanf("%d",&nodevalue[i]);}
        for (int i = 1; i <= vertex - 1; i++) {
            ...
            y++;
             struct edge e;
            e.v=y;/// connected tunnel
            G[x].push_back(e);
            e.v=x;/// connected tunnel
            G[y].push_back(e);
        }
        build_hld();
        int Q,t;
